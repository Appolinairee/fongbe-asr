...
final class zzaty {
    ...
    static String zzb(byte[] bArr, String str) throws GeneralSecurityException, UnsupportedEncodingException {
        ...
        if (zza2 == null) {
            ...
            bArr2 = ((zzatn) zza2.zzbr()).zzaV();
        }
        return zzatr.zza(bArr2, true);
    }
    ...
    static void zze() {
        synchronized (zzf) {
            if (!zza) {
                zza = true;
                new Thread(new zzatw((zzatx) null)).start();
            }
        }
    }
    ...
    private static byte[] zzh(byte[] bArr, String str, boolean z) throws NoSuchAlgorithmException, UnsupportedEncodingException {
        ...
        zzatz[] zzatzArr = new zzaun().zzcG;
        int length3 = zzatzArr.length;
        for (int i3 = 0; i3 < 12; i3++) {
            zzatzArr[i3].zza(bArr2, bArr4);
        }
        if (str != null && str.length() > 0) {
            if (str.length() > 32) {
                str = str.substring(0, 32);
            }
            new zzatp(str.getBytes("UTF-8")).zza(bArr4);
        }
        return bArr4;
    }
}
