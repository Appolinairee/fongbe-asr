...
public final class zztk implements zzue, zzud {
    ...
    public zztk(zzue zzue, boolean z, long j, long j2) {
        this.zza = zzue;
        this.zzb = j2;
    }
    ...
    public final long zza(long j, zzlp zzlp) {
        ...
        long max = Math.max(0, Math.min(zzlp.zzc, j));
        long j2 = zzlp.zzd;
        long j3 = this.zzb;
        long max2 = Math.max(0, Math.min(j2, j3 == Long.MIN_VALUE ? Long.MAX_VALUE : j3 - j));
        if (!(max == zzlp.zzc && max2 == zzlp.zzd)) {
            zzlp = new zzlp(max, max2);
        }
        return this.zza.zza(j, zzlp);
    }
