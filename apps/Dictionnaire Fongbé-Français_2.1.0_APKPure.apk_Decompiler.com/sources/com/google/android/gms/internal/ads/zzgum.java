...
public final class zzgum implements zzgdn {
    ...
    public static zzgdn zzb(zzgfn zzgfn) throws GeneralSecurityException {
        if (zzgks.zza(1)) {
            return new zzgum(zzgfn.zzd().zzd(zzgdw.zza()), zzgfn.zzb().zzb(), zzgfn.zzc().zzc());
        }
        throw new GeneralSecurityException("Can not use AES-EAX in FIPS-mode.");
    }
    ...
    public final byte[] zza(byte[] bArr, byte[] bArr2) throws GeneralSecurityException {
        ...
        if (length2 < 0) {
            ...
            byte[] bArr4 = this.zze;
            byte[] zze2 = zze(cipher, 0, bArr, bArr4.length, this.zzg);
            if (bArr2 == null) {
                bArr2 = new byte[0];
            }
            byte[] bArr5 = bArr2;
            byte[] zze3 = zze(cipher, 1, bArr5, 0, bArr5.length);
            byte[] zze4 = zze(cipher, 2, bArr, this.zze.length + this.zzg, length2);
            int i = length - 16;
            byte b = 0;
            for (int i2 = 0; i2 < 16; i2++) {
                b = (byte) (b | (((bArr[i + i2] ^ zze3[i2]) ^ zze2[i2]) ^ zze4[i2]));
            }
            if (b == 0) {
