package com.google.android.gms.internal.ads;

import android.content.Context;
import com.google.common.util.concurrent.ListenableFuture;
import java.io.File;
import java.io.FileInputStream;
import java.io.IOException;
import java.security.MessageDigest;
import java.security.NoSuchAlgorithmException;
import java.util.concurrent.Executor;

/* compiled from: com.google.android.gms:play-services-ads@@23.6.0 */
public final class zzatv {
    private final ListenableFuture zza;
    private final zzavd zzb;
    private final zzary zzc;

    public zzatv(Context context, Executor executor, zzary zzary, zzavd zzavd) {
        this.zzb = zzavd;
        this.zzc = zzary;
        this.zza = zzgch.zzj(new zzatt(this, context), executor);
    }

    private static String zzc(Context context) {
        FileInputStream fileInputStream;
        File file = new File(context.getPackageResourcePath());
        if (!file.exists() || !file.canRead()) {
            return "";
        }
        try {
            fileInputStream = new FileInputStream(file);
            byte[] bArr = new byte[16384];
            MessageDigest instance = MessageDigest.getInstance("SHA256");
            for (int read = fileInputStream.read(bArr); read != -1; read = fileInputStream.read(bArr)) {
                instance.update(bArr, 0, read);
            }
            zzgaa zzf = zzgaa.zzi().zzf();
            byte[] digest = instance.digest();
            String zzj = zzf.zzj(digest, 0, digest.length);
            fileInputStream.close();
            return zzj;
        } catch (IOException | UnsupportedOperationException | NoSuchAlgorithmException unused) {
            return "";
        } catch (Throwable th) {
            th.addSuppressed(th);
        }
        throw th;
    }

    /* access modifiers changed from: private */
    /* JADX WARNING: Can't wrap try/catch for region: R(2:33|34) */
    /* JADX WARNING: Code restructure failed: missing block: B:34:?, code lost:
        r1 = com.google.android.gms.internal.ads.zzgch.zzh("");
     */
    /* JADX WARNING: Missing exception handler attribute for start block: B:33:0x00c3 */
    /* JADX WARNING: Removed duplicated region for block: B:26:0x005e A[SYNTHETIC, Splitter:B:26:0x005e] */
    /* JADX WARNING: Removed duplicated region for block: B:28:0x0063 A[SYNTHETIC, Splitter:B:28:0x0063] */
    /* JADX WARNING: Removed duplicated region for block: B:38:0x00d4  */
    /* JADX WARNING: Removed duplicated region for block: B:44:0x00e5 A[SYNTHETIC, Splitter:B:44:0x00e5] */
    /* JADX WARNING: Removed duplicated region for block: B:50:? A[RETURN, SYNTHETIC] */
    /* renamed from: zzd */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    public final java.lang.String zzb(android.content.Context r12) {
        /*
            r11 = this;
            com.google.android.gms.internal.ads.zzavd r0 = r11.zzb
            r1 = 31
            java.lang.String r2 = "E"
            if (r0 == 0) goto L_0x004b
            com.google.android.gms.internal.ads.zzary r0 = r11.zzc
            boolean r0 = r0.zzf()
            if (r0 != 0) goto L_0x004b
            com.google.android.gms.internal.ads.zzavd r0 = r11.zzb
            com.google.common.util.concurrent.ListenableFuture r0 = r0.zza()
            if (r0 == 0) goto L_0x004b
            int r0 = android.os.Build.VERSION.SDK_INT
            if (r0 < r1) goto L_0x0028
            com.google.android.gms.internal.ads.zzavd r0 = r11.zzb
            com.google.common.util.concurrent.ListenableFuture r0 = r0.zza()
            boolean r0 = r0.isDone()
            if (r0 == 0) goto L_0x004b
        L_0x0028:
            com.google.android.gms.internal.ads.zzavd r0 = r11.zzb     // Catch:{ InterruptedException | ExecutionException | TimeoutException -> 0x004a }
            com.google.common.util.concurrent.ListenableFuture r0 = r0.zza()     // Catch:{ InterruptedException | ExecutionException | TimeoutException -> 0x004a }
            com.google.android.gms.internal.ads.zzary r3 = r11.zzc     // Catch:{ InterruptedException | ExecutionException | TimeoutException -> 0x004a }
            int r3 = r3.zza()     // Catch:{ InterruptedException | ExecutionException | TimeoutException -> 0x004a }
            long r3 = (long) r3     // Catch:{ InterruptedException | ExecutionException | TimeoutException -> 0x004a }
            java.util.concurrent.TimeUnit r5 = java.util.concurrent.TimeUnit.MILLISECONDS     // Catch:{ InterruptedException | ExecutionException | TimeoutException -> 0x004a }
            java.lang.Object r0 = r0.get(r3, r5)     // Catch:{ InterruptedException | ExecutionException | TimeoutException -> 0x004a }
            com.google.android.gms.internal.ads.zzasy r0 = (com.google.android.gms.internal.ads.zzasy) r0     // Catch:{ InterruptedException | ExecutionException | TimeoutException -> 0x004a }
            if (r0 == 0) goto L_0x004b
            boolean r3 = r0.zzaj()     // Catch:{ InterruptedException | ExecutionException | TimeoutException -> 0x004a }
            if (r3 == 0) goto L_0x004b
            java.lang.String r0 = r0.zzh()     // Catch:{ InterruptedException | ExecutionException | TimeoutException -> 0x004a }
            goto L_0x004c
        L_0x004a:
        L_0x004b:
            r0 = r2
        L_0x004c:
            boolean r3 = r0.equals(r2)
            if (r3 == 0) goto L_0x00d7
            int r3 = android.os.Build.VERSION.SDK_INT
            if (r3 >= r1) goto L_0x0058
            goto L_0x00d7
        L_0x0058:
            int r3 = android.os.Build.VERSION.SDK_INT     // Catch:{ InterruptedException | ExecutionException -> 0x00d6 }
            java.lang.String r4 = ""
            if (r3 >= r1) goto L_0x0063
            com.google.common.util.concurrent.ListenableFuture r1 = com.google.android.gms.internal.ads.zzgch.zzh(r4)     // Catch:{ InterruptedException | ExecutionException -> 0x00d6 }
            goto L_0x00c7
        L_0x0063:
            java.lang.String r6 = r12.getPackageName()     // Catch:{ all -> 0x00c3 }
            java.lang.String r1 = "X.509"
            java.security.cert.CertificateFactory r1 = java.security.cert.CertificateFactory.getInstance(r1)     // Catch:{ all -> 0x00c3 }
            com.google.android.gms.internal.ads.zzgaa r3 = com.google.android.gms.internal.ads.zzgaa.zzi()     // Catch:{ all -> 0x00c3 }
            com.google.android.gms.internal.ads.zzgaa r3 = r3.zzf()     // Catch:{ all -> 0x00c3 }
            java.lang.String r5 = ""
            byte[] r3 = r3.zzk(r5)     // Catch:{ all -> 0x00c3 }
            java.util.ArrayList r9 = new java.util.ArrayList     // Catch:{ all -> 0x00c3 }
            r9.<init>()     // Catch:{ all -> 0x00c3 }
            java.io.ByteArrayInputStream r5 = new java.io.ByteArrayInputStream     // Catch:{ all -> 0x00c3 }
            r5.<init>(r3)     // Catch:{ all -> 0x00c3 }
            java.security.cert.Certificate r3 = r1.generateCertificate(r5)     // Catch:{ all -> 0x00c3 }
            r9.add(r3)     // Catch:{ all -> 0x00c3 }
            java.lang.String r3 = android.os.Build.TYPE     // Catch:{ all -> 0x00c3 }
            java.lang.String r5 = "user"
            boolean r3 = r3.equals(r5)     // Catch:{ all -> 0x00c3 }
            if (r3 != 0) goto L_0x00b0
            com.google.android.gms.internal.ads.zzgaa r3 = com.google.android.gms.internal.ads.zzgaa.zzi()     // Catch:{ all -> 0x00c3 }
            com.google.android.gms.internal.ads.zzgaa r3 = r3.zzf()     // Catch:{ all -> 0x00c3 }
            java.lang.String r5 = ""
            byte[] r3 = r3.zzk(r5)     // Catch:{ all -> 0x00c3 }
            java.io.ByteArrayInputStream r5 = new java.io.ByteArrayInputStream     // Catch:{ all -> 0x00c3 }
            r5.<init>(r3)     // Catch:{ all -> 0x00c3 }
            java.security.cert.Certificate r1 = r1.generateCertificate(r5)     // Catch:{ all -> 0x00c3 }
            r9.add(r1)     // Catch:{ all -> 0x00c3 }
        L_0x00b0:
            com.google.android.gms.internal.ads.zzatu r1 = new com.google.android.gms.internal.ads.zzatu     // Catch:{ all -> 0x00c3 }
            r1.<init>()     // Catch:{ all -> 0x00c3 }
            android.content.pm.PackageManager r5 = r12.getPackageManager()     // Catch:{ all -> 0x00c3 }
            r7 = 0
            r8 = 8
            r10 = r1
            r5.requestChecksums(r6, r7, r8, r9, r10)     // Catch:{ all -> 0x00c3 }
            com.google.android.gms.internal.ads.zzgdb r1 = r1.zza     // Catch:{ all -> 0x00c3 }
            goto L_0x00c7
        L_0x00c3:
            com.google.common.util.concurrent.ListenableFuture r1 = com.google.android.gms.internal.ads.zzgch.zzh(r4)     // Catch:{ InterruptedException | ExecutionException -> 0x00d6 }
        L_0x00c7:
            java.lang.Object r1 = r1.get()     // Catch:{ InterruptedException | ExecutionException -> 0x00d6 }
            java.lang.String r1 = (java.lang.String) r1     // Catch:{ InterruptedException | ExecutionException -> 0x00d6 }
            boolean r3 = com.google.android.gms.internal.ads.zzfve.zzd(r1)     // Catch:{ InterruptedException | ExecutionException -> 0x00d6 }
            r4 = 1
            if (r4 == r3) goto L_0x00d7
            r0 = r1
            goto L_0x00d7
        L_0x00d6:
        L_0x00d7:
            boolean r1 = r0.equals(r2)
            if (r1 == 0) goto L_0x00e9
            com.google.android.gms.internal.ads.zzary r1 = r11.zzc
            boolean r1 = r1.zzd()
            if (r1 != 0) goto L_0x00e9
            java.lang.String r0 = zzc(r12)     // Catch:{ ClassCastException -> 0x00e9 }
        L_0x00e9:
            return r0
        */
        throw new UnsupportedOperationException("Method not decompiled: com.google.android.gms.internal.ads.zzatv.zzb(android.content.Context):java.lang.String");
    }

    public final ListenableFuture zza() {
        return this.zza;
    }
}
