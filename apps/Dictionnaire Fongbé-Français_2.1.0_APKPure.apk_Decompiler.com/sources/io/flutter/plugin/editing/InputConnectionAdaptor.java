...
public class InputConnectionAdaptor extends BaseInputConnection implements ListenableEditingState.EditingStateWatcher {
    ...
    public InputConnectionAdaptor(View view, int i, TextInputChannel textInputChannel2, ScribeChannel scribeChannel2, KeyboardDelegate keyboardDelegate2, ListenableEditingState listenableEditingState, EditorInfo editorInfo, FlutterJNI flutterJNI) {
        super(view, true);
        ListenableEditingState listenableEditingState2 = listenableEditingState;
        ...
        this.scribeChannel = scribeChannel2;
        this.mEditable = listenableEditingState2;
        listenableEditingState2.addEditingStateListener(this);
        this.mEditorInfo = editorInfo;
        this.keyboardDelegate = keyboardDelegate2;
        this.flutterTextUtils = new FlutterTextUtils(flutterJNI);
        this.mLayout = new DynamicLayout(listenableEditingState2, new TextPaint(), Integer.MAX_VALUE, Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
        this.mImm = (InputMethodManager) view.getContext().getSystemService("input_method");
    }

    public InputConnectionAdaptor(View view, int i, TextInputChannel textInputChannel2, ScribeChannel scribeChannel2, KeyboardDelegate keyboardDelegate2, ListenableEditingState listenableEditingState, EditorInfo editorInfo) {
        this(view, i, textInputChannel2, scribeChannel2, keyboardDelegate2, listenableEditingState, editorInfo, new FlutterJNI());
    }
    ...
    public ExtractedText getExtractedText(ExtractedTextRequest extractedTextRequest, int i) {
        ...
        if (z2 == z) {
            Log.d(TAG, "The input method toggled text monitoring ".concat(z2 ? DebugKt.DEBUG_PROPERTY_VALUE_ON : DebugKt.DEBUG_PROPERTY_VALUE_OFF));
        }
    ...
    public boolean commitContent(InputContentInfo inputContentInfo, int i, Bundle bundle) {
        if (Build.VERSION.SDK_INT >= 25 && (i & 1) != 0) {
            try {
                FileVisitorImpl$$ExternalSyntheticApiModelOutline0.m(inputContentInfo);
                if (FileVisitorImpl$$ExternalSyntheticApiModelOutline0.m(inputContentInfo).getMimeTypeCount() > 0) {
                    FileVisitorImpl$$ExternalSyntheticApiModelOutline0.m(inputContentInfo);
                    Uri m = FileVisitorImpl$$ExternalSyntheticApiModelOutline0.m(inputContentInfo);
                    String mimeType = FileVisitorImpl$$ExternalSyntheticApiModelOutline0.m(inputContentInfo).getMimeType(0);
