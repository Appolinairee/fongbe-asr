...
public class TextInputPlugin implements ListenableEditingState.EditingStateWatcher {
    ...
    public TextInputPlugin(View view, TextInputChannel textInputChannel2, ScribeChannel scribeChannel2, PlatformViewsController platformViewsController2) {
        this.mView = view;
        this.mEditable = new ListenableEditingState((TextInputChannel.TextEditState) null, view);
        this.mImm = (InputMethodManager) view.getContext().getSystemService("input_method");
        if (Build.VERSION.SDK_INT >= 26) {
            this.afm = FileVisitorImpl$$ExternalSyntheticApiModelOutline0.m(view.getContext().getSystemService(FileVisitorImpl$$ExternalSyntheticApiModelOutline0.m()));
        } else {
            this.afm = null;
        }
    ...
    public void saveEditableSizeAndTransform(double d, double d2, double[] dArr) {
        ...
        double d13 = dArr3[3];
        valueOf.getClass();
        this.lastClientRect = new Rect(i, (int) (d11 * d10), (int) Math.ceil(d12 * d10), (int) Math.ceil(d13 * d10));
    }
    ...
    public void onProvideAutofillVirtualStructure(ViewStructure viewStructure, int i) {
        ...
        if (Build.VERSION.SDK_INT >= 26 && needsAutofill()) {
            String str = this.configuration.autofill.uniqueIdentifier;
            AutofillId m = viewStructure.getAutofillId();
            for (int i2 = 0; i2 < this.autofillConfiguration.size(); i2++) {
                ...
                if (autofill != null) {
                    FileVisitorImpl$$ExternalSyntheticApiModelOutline0.m(viewStructure2, 1);
                    ViewStructure m2 = FileVisitorImpl$$ExternalSyntheticApiModelOutline0.m(viewStructure2, i2);
                    m2.setAutofillId(m, keyAt);
                    if (autofill.hints.length > 0) {
                        m2.setAutofillHints(autofill.hints);
                    }
                    FileVisitorImpl$$ExternalSyntheticApiModelOutline0.m(m2, 1);
                    m2.setVisibility(0);
                    if (autofill.hintText != null) {
                        m2.setHint(autofill.hintText);
                    }
                    if (str.hashCode() != keyAt || (rect = this.lastClientRect) == null) {
                        ViewStructure viewStructure3 = m2;
                        viewStructure3.setDimens(0, 0, 0, 0, 1, 1);
                        viewStructure3.setAutofillValue(AutofillValue.forText(autofill.editState.text));
                    } else {
                        m2.setDimens(rect.left, this.lastClientRect.top, 0, 0, this.lastClientRect.width(), this.lastClientRect.height());
                        m2.setAutofillValue(AutofillValue.forText(this.mEditable));
                    }
                }
            }
        }
        ...
        if (Build.VERSION.SDK_INT >= 26 && (configuration2 = this.configuration) != null && this.autofillConfiguration != null && configuration2.autofill != null) {
            ...
            for (int i = 0; i < sparseArray.size(); i++) {
                ...
                if (!(configuration3 == null || configuration3.autofill == null)) {
                    ...
                    String charSequence = FileVisitorImpl$$ExternalSyntheticApiModelOutline0.m((Object) sparseArray.valueAt(i)).getTextValue().toString();
