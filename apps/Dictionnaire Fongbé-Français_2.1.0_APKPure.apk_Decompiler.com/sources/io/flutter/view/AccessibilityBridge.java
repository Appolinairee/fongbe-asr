...
public class AccessibilityBridge extends AccessibilityNodeProvider {
    ...
    private boolean shouldSetCollectionInfo(SemanticsNode semanticsNode) {
        return semanticsNode.scrollChildren > 0 && (SemanticsNode.nullableHasAncestor(this.accessibilityFocusedSemanticsNode, new AccessibilityBridge$$ExternalSyntheticLambda3(semanticsNode)) || !SemanticsNode.nullableHasAncestor(this.accessibilityFocusedSemanticsNode, new AccessibilityBridge$$ExternalSyntheticLambda2()));
    }

    private void setBoldTextFlag() {
        View view = this.rootAccessibilityView;
        if (view != null && view.getResources() != null) {
            int m = this.rootAccessibilityView.getResources().getConfiguration().fontWeightAdjustment;
            if (m == Integer.MAX_VALUE || m < BOLD_TEXT_WEIGHT_ADJUSTMENT) {
    ...
    public AccessibilityNodeInfo createAccessibilityNodeInfo(int i) {
        ...
        if (semanticsNode.platformViewId == -1 || !this.platformViewsAccessibilityDelegate.usesVirtualDisplay(semanticsNode.platformViewId)) {
            ...
            if (semanticsNode.hasFlag(Flag.IS_TEXT_FIELD)) {
                ...
                CharSequence access$3400 = semanticsNode.getValueLabelHint();
                ...
                if (str2 != null) {
                    obtainAccessibilityNodeInfo2.setContentDescription(str2);
                }
            }
    ...
    private void onWindowNameChange(SemanticsNode semanticsNode) {
        ...
        obtainAccessibilityEvent.getText().add(access$6800);
        sendAccessibilityEvent(obtainAccessibilityEvent);
    }
    ...
    private boolean doesLayoutInDisplayCutoutModeRequireLeftInset() {
        ...
        int m = activity.getWindow().getAttributes().layoutInDisplayCutoutMode;
        if (m == 2 || m == 0) {
            return true;
        }
        return false;
